...
exports.login = (req, res) => {
  ...
  User.findOne({ email }, (error, foundUser) => {
    if (error) {
      ...
      // return res.status(422).send({
      //   errors: [
      //     { title: 'DB Error', detail: 'Oops, something went wrong!' },
      //   ],
      // });
    }
...
exports.register = (req, res) => {
  ...
  User.findOne({ email }, (error, existingUser) => {
    if (error) {
      return res.mongoError(error);
      // return res.status(422).send({
      //   errors: [{ title: 'DB Error', detail: 'Oops, something went wrong!' }],
      // });
    }
  ...
  if (token) {
    ...
    User.findById(decodedToken.sub, (error, foundUser) => {
      if (error) {
        return res.status(422).send({
          errors: [
            { title: 'DB Error', detail: 'Oooops, something went wrong!' },
          ],
        });
      }
...
//           {
//             title: 'DataBase Error',
//             detail: 'Token is Malformed Bozo',
//           },
//         ],
//       });
//     }
